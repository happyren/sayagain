...
async function renderServers(): Promise<void> {
  ...
  $("#servers").innerHTML =
    `<p>daemon ${esc(health.version)} · pid ${esc(health.pid)} · store ${esc(health.ledger)} · spans ${health.otlp ? `to ${esc(health.otlp)}` : "not exported"} · holds ${health.hold === "never" ? "off: observing, nothing waits for you (sayagain up --hold turns them on)" : "on (sayagain up --observe turns them off)"}</p>` +
    table(
